...
export async function addItemToCart(req, res) {
  ...
  try {
    ...
    // find cart by user id within the transaction
    ...
    const [ci, created] = await CartItem.findOrCreate({
      ...
      defaults: { cart_id: cart.id, item_id: itemId },
      transaction: t,
    });
    ...
    await t.commit();
    const updated = await Cart.findByPk(cart.id, { include: [{ model: Item }] });
...
export async function removeItemFromCart(req, res) {
  try {
    ...
    const ci = await CartItem.findOne({ where: { cart_id: cart.id, item_id: itemId } });
...
export async function getCartTotalPrice(req, res) {
  try {
    ...
    const totalPrice = Math.round((total + Number.EPSILON) * 100) / 100;
